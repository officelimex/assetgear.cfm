<cfoutput> 
    ...
    <cfdocument pagetype="a4" format="pdf" margintop="0" marginbottom="0" marginleft="0" marginright="0" backgroundvisible="yes">
      ...
      <body>
    <cfquery name="qz">
      SELECT * FROM ptw_permit p
      INNER JOIN ptw_jha j ON j.JHAId = p.JHAId
      INNER JOIN work_order w ON w.WorkOrderId = j.WorkOrderId
      WHERE w.DepartmentId = 16 AND YEAR(p.Date) = "2019" 
      ORDER BY p.PermitId 
    </cfquery>

    <cfloop query="qz">
   
      <cfset qP = application.com.Permit.GetPermit(qz.PermitId)/>
      <cfset qGT = application.com.Permit.GetGasTest(qz.PermitId)/>
    ...
    <cfset request.letterhead.Id="PTW ## #qz.PermitId#"/>
    ...
    <cfquery name="qPV">
        ...
        WHERE prv.PermitId = <cfqueryparam cfsqltype="cf_sql_integer" value="#qz.PermitId#"/>
    </cfquery>
    <cfloop query="qPV">          
      <tr>
