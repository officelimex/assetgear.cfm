...
var aGrid = new Class({
		...
		**/
		newCommandButton : function(cmd, tr, row)	{
			var eTxt = '', cmdText='', eType='', if_condition = '', _butt = null,titl='';
			...
			if(if_condition=='' || eval(if_condition))	{// not defined
				_butt = new Element('a', {
					'class' : cmd['class'],
					html : eTxt,
					title:titl,
		...
		*/
		createCommand : function (row, tr, i) {
			...
			this.options.commands.each(function (cmdb, index) {
				var cmd = cmdb;
				var eType = 'a', cmdid = cmd['id'], cmdText = '', cmdIcon = '', eTxt = '', cmdPin = cmd['pin'];
				...
				if(typeOf(cmd['command'])==='null')	{
					...
					var but_ = new Element('button', {
						...
						html: '<i class="' + cmd['icon'] + '"></i> ' + cmd['text'] + ' <span class="caret"></span>'
					});
			...
			this.options.headers.each(function (hd, _index1) { 
				...
				if (hdLen == _index1) {
					...
					var btn = new aButton('', {
							'value' : 'add',
							'click' : function (e) {
								var _me = e.target, data_ = "", req_field = "", field_name = "";
								for (var _i = 0; _i <= hdLen - 1; ++_i) {
									data_ = data_ + '&' + thisClass.options.headers[_i]['type'] + _i + '=' + encodeURIComponent($(unid + _i).value);
									field_name = field_name + "`" + thisClass.options.headers[_i]['title'];
									if(thisClass.options.headers[_i]['required'])	{
										req_field = req_field + "," +  thisClass.options.headers[_i]['type'] + _i;
									}
								}
					...
					if (thisClass.options.allowUpdate) {
						...
						bg.adopt(new aButton('', {
								...
								'click' : function (e) {
									...
									case 's':
										var _param = '', ctl_vl, field_name='', req_field='';
