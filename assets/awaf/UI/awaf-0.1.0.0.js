	...
	select: function (el) {
		var tabs = $$(
			"#" +
				this.Id +
				" ." +
				this.options.cls.tabHandle +
				" ." +
				this.options.cls.tab
		);
		...
		var contents = $$(
			"#" +
				this.Id +
				" ." +
				this.options.cls.tabContent +
				" ." +
				this.options.cls.tabPane
		);
			...
			var imgRefresh = new Element("img", {
				...
				events: {
					click: function (e) {
						...
						if (
							confirm(
								"Are you sure you want to refresh the content of this tab?"
							)
						) {
...
var aGrid = new Class({
	...
	createElements: function () {
		...
		var structureHtml =
			"<table id='" +
			this.Id +
			"_t' class='table table-striped " +
			this.options["class"] +
			"'><thead><tr></tr></thead><tfoot><tr><td colspan='" +
			hlen +
			"'><div>&nbsp;<span class='moo-foot-left'></span><span class='search'></span><span class='moo-foot-right'></span></div></td></tr></tfoot><tbody></tbody></table>";
		...
		this.footerLeft = this.table.getElements(
			"tfoot > tr > td > div > span.moo-foot-left"
		)[0];
		this.footerSearch = this.table.getElements(
			"tfoot > tr > td > div > span.search"
		)[0];
		this.searchButton = null;
		this.footerRight = this.table.getElements(
			"tfoot > tr > td > div > span.moo-foot-right"
		)[0];
	...
	reorder: function (id) {
		...
		this.activeColumnEl = this.tableHead
			.getChildren("tr")[0]
			.getChildren("th#" + this.Id + id)[0];
	...
	/**	
			@cmd object
			@tr object table row
			@row number current record
			return object
		**/
	newCommandButton: function (cmd, tr, row) {
		var eTxt = "",
			cmdText = "",
			eType = "",
			if_condition = "",
		...
		if (if_condition == "" || eval(if_condition)) {
			// not defined
			_butt = new Element("a", {
				class: cmd["class"],
				html: eTxt,
				...
				events: {
					click: function (e) {
						e.stopPropagation();
						this.fireEvent(cmd["id"] + "click", [tr, row, e]);
					}.bind(this),
				},
			});
		} else {
	...
	createCommand: function (row, tr, i) {
		...
		var dGroup2 = new Element("div", { class: "btn-group" });
		var dv = new Element("div", {
			class: "btn-toolbar",
		});
		this.options.commands.each(function (cmdb, index) {
			var cmd = cmdb;
			var eType = "a",
				cmdid = cmd["id"],
				cmdText = "",
			...
			if (typeOf(cmd["command"]) === "null") {
				// if the cmd button is pinned
				if (cmdPin && this.pincounter == 1) {
					var pin_id = $(this.Id + "_pinned_cmd");
					pin_id.adopt(this.newCommandButton(cmd, tr, row));
				}
				// for other buttons
				else if (!cmdPin) {
					iCmd = this.newCommandButton(cmd, tr, row);
				...
				// gorup command
				var dGroup = new Element("div", { class: "btn-group" });
				var ul = new Element("ul", { class: "dropdown-menu" });
				//var bx = new Element('button',{});
				var but_ = new Element("button", {
					class: "btn btn-small " + cmd["class"] + " dropdown-toggle",
					//'data-toggle':"dropdown",
					events: {
						blur: function (e) {
							(function () {
								_self.searchButton.focus();
								dGroup.removeClass("open");
							}).delay(300);
						}.bind(this),
						click: function (e) {
							//e.stopPropagation();
							dGroup.toggleClass("open");
							e.target.focus();
						}.bind(this),
					},
					html:
				...
				/*dGroup.addEvents({
						'click': function (e) {
							e.stop();
							alert('');
							//dGroup.addClass('open');
							//but_.focus();	console.log(dGroup);							
						}.bind(this), 
					});*/
				dGroup.adopt(but_);

				cmd["command"].each(function (cmdb, index) {
					cmdb["class"] = "";
					switch (cmdb["type"]) {
						case "divider":
							ul.adopt(new Element("li", { class: "divider" }));
							break;
						default:
							iCmd = this.newCommandButton(cmdb, tr, row);
							if (typeOf(iCmd) == "element") {
								ul.adopt(new Element("li").adopt(iCmd));
							}
					}
				}, this);
				dGroup.adopt(ul);
		...
		return cmdEl.adopt(dv.adopt(dGroup2));
	},
...
var aWindow = new Class({
	...
	initialize: function (id, options) {
		...
		if (typeOf($(id)) == "element") {
			...
			this.windowInnerFrame = this._createWindowInnerFrame().adopt(
				this.windowContentFrame
			);

			this.windowFrame = this._createWindowFrame(id);
			this.windowFrame.adopt(
				this.titleBar,
				this.windowInnerFrame,
				this.buttonFrame
			);
	...
	_makewindowActive: function (_id) {
		$$("#" + this.options.renderTo.get("id") + " .dbx-frame").setStyle(
			"z-index",
			10000000
		);
		$$(
			"#" +
				this.options.renderTo.get("id") +
				" .dbx-frame .dbx-titlebar .dbx-ctl .close"
		).addClass("cls-dis");
	...
	_setPosition: function () {
		...
		if (this.options.position.x == null) {
			this.options.position.x =
				150 + bsize.x / 2 - parseInt(this.options.size.width) / 2;
		}
	...
	requestData: function (u) {
		new Request.HTML({
			...
			onFailure: function (e) {
				...
				var re = new RegExp("<s*h1[^>]*>(.*?)<s*/s*h1>", "gmi");
				u.innerHTML =
					'<div style="color:red">Error on request:\r\r' +
					re.exec(d)[1] +
					"</div>";
			},
		}).send();
			...
			events: {
				click: function (e) {
					...
					if (
						...
						new Request({
							url:
								"assets/awaf/tags/xUploader_1000/deletefile.cfm?tempfolder=" +
				...
				click: function (e) {
					...
					xhrObj.upload.addEventListener(
						...
						function () {
							...
							//dbtn.disabled = false;
							dbtn.setStyle("display", "inline-block");
					...
					xhrObj.open(
						"POST",
						"assets/awaf/tags/xUploader_1000/upload.cfm?tempfolder=" +
							this.options.tempfolder,
						true
					);
...
var aEditTable = new Class({
	...
	options: {
		headers: [], // header : [{title:'',type:'', size, data:[[][]]},{},events:{}]
			...
			if (hdLen == _index1) {
					...
					click: function (e) {
						...
						for (var _i = 0; _i <= hdLen - 1; ++_i) {
							data_ =
								data_ +
								"&" +
								thisClass.options.headers[_i]["type"] +
								_i +
								"=" +
								encodeURIComponent($(unid + _i).value);
							field_name =
								field_name + "`" + thisClass.options.headers[_i]["title"];
							/* 							if (thisClass.options.headers[_i]["required"]) {
								req_field =
									req_field + "," + thisClass.options.headers[_i]["type"] + _i;
							} else {
								other_field =
									other_field +
									"," +
							...
							req_field =
								...
								_i +
								"-" +
								thisClass.options.headers[_i]["required"];
						}
						field_name = encodeURIComponent(field_name);
						data_ = data_ + "&rf=" + req_field + "&fn=" + field_name;

						new Request({
							url:
								"assets/awaf/tags/xEditTable/save_data.cfm?" +
								data_ +
								"&session=" +
								thisClass.options.sessionId,
							method: "post",
							onRequest: function () {
								_me.disabled = true;
								_me.value = "...";
				...
				if (ctlInput.get("tag") == "select") {
					...
					new Meio.Autocomplete.Select.One(ctlInput, {
						//onSelect: function(x,e){eval(hd['onSelect'])}
					});
				}
				i_row.adopt(incont);
			}
							...
							click: function (e) {
								...
								switch (_me.value) {
									...
									case "Save":
										...
										for (
											var _i = 0;
											_i < thisClass.options.headers.length - 1;
											++_i
										) {
											...
											_param =
												...
												_i +
												"=" +
												encodeURIComponent(ctl_vl);
											field_name =
												field_name +
												"`" +
												thisClass.options.headers[_i]["title"];
											req_field =
												req_field +
												"," +
												thisClass.options.headers[_i]["type"] +
										...
										field_name = encodeURIComponent(field_name);
										_param = _param + "&rf=" + req_field + "&fn=" + field_name;
										new Request({
											url:
												"assets/awaf/tags/xEditTable/save_data.cfm?id=" +
												_id +
												_param +
												"&session=" +
												thisClass.options.sessionId,
