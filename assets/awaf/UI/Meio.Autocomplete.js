...
(function(global, $){
	...
	Meio.Autocomplete.Select.One = new Class({
		...
		initialize: function(select, options, listInstance){
			this.setOptions(options); // options was no set {Arowolo}
			this.select = $(select);
			this.select_id = this.select.get('id'); // Arowolo
			this.replaceSelect();
		...
		defaults: function(keys){
			...
			return {
				...
				formatItem: function(text, data, i){
					return text ? self._getValueFromKeys(data, keys).replace(new RegExp('(' + text.escapeRegExp() + ')', 'gi'), '<span class="ma-filter">$1</span>') : self._getValueFromKeys(data, keys);
				}
			};
		},
