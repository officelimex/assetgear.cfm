...
<tr>
  <td>
    ...
    <cfquery name="qT">
        SELECT
            mii.currency,sum(mii.UnitPrice*mii.Quantity) UnitPrice
        FROM whs_issue_item mii
        WHERE mii.IssueId = <cfqueryparam cfsqltype="cf_sql_integer" value="#url.id#"/>
        GROUP BY mii.Currency
    </cfquery>
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tbl">
      <tr>
        ...
        <th width="83" nowrap="nowrap" class="a-right">Unit Price</th>
      ...
      <cfloop query="qMII">
        <tr <cfif qMII.Currentrow eq qMII.Recordcount> class="bottom" </cfif>>
          ...
          <td valign="top">[#qMII.Code#] #qMII.Item# </td>
          <td width="80" align="right" valign="top" class="no-right">#qMII.Quantity#</td>
          <td width="61" valign="top">#qMII.UM#</td>
          <td align="right" valign="top">
          		#Numberformat(val(qMII.UnitPrice),'9,999.99')#
          </td>
          <td align="right" valign="top" nowrap="nowrap" class="cbg">
          <cfset ctv = qMII.Quantity*val(qMII.UnitPrice)/>
          <cfset ts=ts+ctv/>
          <cfset gsign = getCurrency(qMII.Currency)/>
               	#gsign# #NumberFormat(ctv,'9,999.99')#</td>
