...
<cfset EXCHANGE_RATE = 150/>
<cfset dmonth = month(dateAdd('d',-20,now()))/>
<cfset dpart = year(now()) & "/10"/>
<cfset startd =  "#dpart#/1">
<cfset dinmonth = DaysInMonth(startd)/>
<cfset endd =  "2015/10/31">
<cfset ystart = year(startd) & "/07/01"/>
...
<cfset request.letterhead.date = "Period : #dateformat(endd,'mmmm')#"/>
...
<tr>
  <td class="noline" align="center">
	...
	<cfquery name="qC1" cachedwithin="#CreateTime(1,0,0)#">
		SELECT 
			DATE_FORMAT(DateOpened,'%M') MonthOpen, DATE_FORMAT(DateOpened,'%m') mc, COUNT(WorkOrderId) WCount 
		FROM work_order
...
<br>
	...
	<cfchart format="png" scalefrom="0" seriesplacement="stacked" show3d="no" chartWidth="530" chartHeight="280"> 
